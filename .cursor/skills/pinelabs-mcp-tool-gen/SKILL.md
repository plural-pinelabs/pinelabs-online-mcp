...
def register_{resource}_tools(
    ...
    async def tool_name(required_param: str) -> str:
        ...
        try:
            result = await client.get(routes.RESOURCE_GET.format(resource_id=rid))
...
2. Add entry to `_TOOLSETS` dict with access type (`"read"`, `"write"`, or `"readwrite"`)
3. The registration function is called automatically by `register_all_tools()`
